...
# Sort and display top correlations by absolute value
...
poly_combinations = {
    'x': [1],           # Original features only
    'x²': [2],          # Squared features only
    'x³': [3],          # Cubic features only
    'x + x²': [1, 2],   # Original + squared
    'x + x³': [1, 3],   # Original + cubic
    'x² + x³': [2, 3],  # Squared + cubic
    'x + x² + x³': [1, 2, 3]  # All features
}
...
for feature_count in [2, 3, 4]:
    ...
    for combo_name, degree_list in poly_combinations.items():
        print(f"Training model with {combo_name}...")
